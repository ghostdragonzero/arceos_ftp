...
const UART_BASE: PhysAddr = pa!(axconfig::devices::UART2_PADDR);

pub static UART2: SpinNoIrq<Pl011Uart> =
    SpinNoIrq::new(Pl011Uart::new(phys_to_virt(UART_BASE).as_mut_ptr()));


pub fn init() {
    let virt = phys_to_virt(UART_BASE).as_mut_ptr();
    info!("init: Pl011Uart base addr: {:p}", virt);
    UART2.lock().init();
    info!("init: UART2 initialized");
}
